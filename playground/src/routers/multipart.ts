...
multipartRouter.static('./upload-demo', {
    cache: true,
    maxAge: 3600,
    etag: true,
    lastModified: true,
...
multipartRouter.static('./uploads', {
...
multipartRouter.get('/', (req, res) => {
    try {
        ...
        try {
            const listResult = multipartRouter.listFiles(uploadsDir);
...
multipartRouter.delete('/delete/{*filepath}', (req, res) => {
    ...
    try {
        ...
        const deleteResult = multipartRouter.deleteFile(filepath, uploadsDir);
...
multipartRouter.get('/files', (req, res) => {
    try {
        const path = require('path');
        const uploadsDir = path.join(__dirname, '../../uploads');
